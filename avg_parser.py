...
def parser(source, label=None):
    ...
    ast_top = []
    for line in lines:
        speakers, tags = parse_tags(ast_top, ast_map, cname_name, name_cname, line)
...
def parse_tags(ast_top, ast_map, cname_name, name_cname, line):
    head = line[:line.find(':')]
    head2 = re.sub('<(\\S+?)>(.+?)</\\1>', '', head)

    # Parse All tag by regex match
    full_tags = re.finditer('<(\\S+?)>(.+?)</\\1>', head)
    half_tags = re.finditer('<(\\S+?)>', head2)
    ...
    for tag in tags:
        match tag['key']:
            ...
            case 'BIN':
                bg_result = tag['value']
                bg_img = IMG[int(bg_result.strip(' '))]
